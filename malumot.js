...
function renderLongestDebtorsTable(debtors) {
...
function renderDebtChart(debtors) {
  const days = [];
  const now = new Date();
  for (let i = 6; i >= 0; i--) {
...
function renderRatingChart(debtors) {
  const rating = debtors.map(d => {
    let total = 0;
    (d.history || []).forEach(h => {
      if (h.type === 'add') total += h.amount;
      if (h.type === 'sub') total -= h.amount;
